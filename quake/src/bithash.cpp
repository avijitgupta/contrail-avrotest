...
//Avijit
void bithash::binary_file_output_2(char* outf) {
  unsigned long long mysize = (unsigned long long)bits.size() / 64ULL;
  long long int flag = 1;
  ofstream ofs(outf);
  for(unsigned long long i = 0; i < mysize; i++) {
    long long int temp = 0;
    for(unsigned int j = 0; j < 64; j++) { // read 8 bits from the bitset
      temp <<= 1;
      if(bits[i*64 + j])
        temp |= flag;
    }
