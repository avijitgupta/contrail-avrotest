...
/**
 * NLineInputFormat which splits N lines of input as one split. In many
 * "pleasantly" parallel applications, each process/mapper processes the same
 * input file (s), but with computations are controlled by different
 ...
 * dataset is specified via a config variable in JobConf.). The NLineInputFormat
 * can be used in such applications, that splits the input file such that by
...
public class NLineInputFormat extends FileInputFormat<LongWritable, Text> {
    ...
    public static List<FileSplit> getSplitsForFile(FileStatus status, Configuration conf,
        int numLinesPerSplit) throws IOException {
        ...
        try {
            ...
            while ((num = lr.readLine(line)) > 0) {
                ...
                if (numLines == numLinesPerSplit) {
                    ...
                    // reads (and consumes) at least one character out of its
                    // upper split boundary. So to make sure that each mapper
                    // gets N lines, we move back the upper split limits of each
                    // split by one character here.
                    if (begin == 0) {
